...
const RequestSchema = z.object({
  denialText: z
    .string()
    .min(30, "Denial text is too short to analyze")
    .max(20000, "Denial text is too long"),
});
...
function getModel(): GenerativeModel | null {
  ...
  cachedModel = ai.getGenerativeModel({
    ...
    generationConfig: {
      ...
      responseSchema: GEMINI_DENIAL_ANALYSIS_SCHEMA as unknown as Parameters<
...
export async function POST(req: Request) {
  ...
  let raw: string;
  try {
    const result = await model.generateContent(buildAnalyzeUserPrompt(denialText));
  ...
  const validated = DenialAnalysisSchema.safeParse(json);
